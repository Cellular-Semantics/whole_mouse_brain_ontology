...
import template_generation_tools
import template_generation_utils
...
from pcl_id_factory import get_class_id, get_taxonomy_id, get_individual_id, read_taxonomy_details_yaml

current_dir = os.path.dirname(os.path.realpath(__file__))
ALL_BASE_FILES = [os.path.join(current_dir, "./test_data/patterns/data/default/CCN202002013_class_base.tsv"),
                  os.path.join(current_dir, "./test_data/patterns/data/default/CCN201912131_class_base.tsv"),
...
PATH_TAXONOMY_DETAILS = os.path.join(os.path.dirname(os.path.realpath(__file__)), './test_data/taxonomy_details.yaml')
MARKER_PATH = os.path.join(os.path.dirname(os.path.realpath(__file__)), './test_data/markers/CS{}_markers.tsv')
ALLEN_MARKER_PATH = os.path.join(os.path.dirname(os.path.realpath(__file__)), './test_data/markers/CS{}_Allen_markers.tsv')
...
class TemplateGenerationTest(unittest.TestCase):
    ...
    def setUp(self):
        template_generation_utils.TAXONOMY_DETAILS_YAML = PATH_TAXONOMY_DETAILS
        pcl_id_factory.TAXONOMY_DETAILS_YAML = PATH_TAXONOMY_DETAILS
        pcl_id_factory.taxonomies = read_taxonomy_details_yaml()
        pcl_id_factory.taxonomy_ids = [taxon["Taxonomy_id"] for taxon in pcl_id_factory.taxonomies]
        template_generation_tools.MARKER_PATH = str(MARKER_PATH)
        template_generation_tools.ALLEN_MARKER_PATH = str(ALLEN_MARKER_PATH)
